import { z } from "zod";
import { runSetupSchema } from "@sim/engine";
...
export const createRunSchema = z.object({ scenarioSlug: z.string().min(2).max(100), setup: runSetupSchema });
export const commandRequestSchema = z.object({
  commandId: z.string().min(8).max(100), expectedVersion: z.number().int().min(1), type: z.string().min(2).max(100), payload: z.unknown(),
});
