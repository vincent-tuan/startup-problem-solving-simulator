...
import {
  checkpoints, competitorProfiles, externalInputBlobs, marketDossierVersions, marketFacts, marketSources, recoveryCredentials, runAgentTurns, runCommands, runDialogueTurns, runEvents, runExternalInputs,
  runFeatureBlobs, runFeatureHeads, runKernelHeads, runs, runSnapshots, runV10Checkpoints, runV10Events, runV10Snapshots,
  runV10AgentTurns, runV10ExternalInputRefs,
...
const toRun = (row: RunRow): RunRecord => {
  if (!row.headState || row.stateFormat !== "legacy_json") throw new Error("LEGACY_RUN_STATE_MISSING");
  return {
...
type V10FeatureHeadRow = typeof runFeatureHeads.$inferSelect;
function composeV10Run(row: RunRow, kernelRow: typeof runKernelHeads.$inferSelect, featureRows: V10FeatureHeadRow[]): V10RunRecord {
  const features = Object.fromEntries(featureRows.map((feature) => [feature.featureId, {
    version: feature.featureVersion,
    public: feature.publicState,
    private: feature.privateState,
    checksum: feature.checksum,
    updatedAtVersion: feature.updatedAtVersion,
  }]));
  return {
    id: row.id, ownerId: row.ownerId, scenarioVersionId: row.scenarioVersionId, parentRunId: row.parentRunId,
    title: row.title, status: row.status as V10RunRecord["status"], seed: row.seed, engineVersion: row.engineVersion,
    stateFormat: "feature_heads_v10", stateVersion: row.stateVersion, headEventSequence: row.headEventSequence,
    state: { kernel: kernelRow.kernel, manifest: kernelRow.manifest, features }, checksum: kernelRow.overallChecksum,
    createdAt: row.createdAt.toISOString(), updatedAt: row.updatedAt.toISOString(), lastPlayedAt: row.lastPlayedAt.toISOString(),
  };
}
...
const toV10CompetitorTurn = (row: typeof runV10AgentTurns.$inferSelect): V10CompetitorTurnRecord => ({
  id: row.id, runId: row.runId, turnId: row.turnId, firmId: row.firmId,
  status: row.status as V10CompetitorTurnRecord["status"], envelope: row.envelope, plan: row.plan,
  provider: row.provider as V10CompetitorTurnRecord["provider"], model: row.model, promptVersion: row.promptVersion,
  latencyMs: row.latencyMs, inputTokens: row.inputTokens, outputTokens: row.outputTokens,
  fallbackReason: row.fallbackReason, createdAt: row.createdAt.toISOString(), completedAt: row.completedAt?.toISOString() ?? null,
});
...
export class PostgresStore implements RuntimeStore {
  async syncScenarios(definitions: ScenarioDefinition[]) {
    ...
    await db.transaction(async (tx) => {
      for (const definition of definitions) {
        ...
        const versionId = scenarioVersionId(definition);
        const [existingVersion] = await tx.select().from(scenarioVersions).where(eq(scenarioVersions.id, versionId)).limit(1);
        if (existingVersion) {
          assertScenarioVersionTransition(existingVersion.content, definition);
          if (existingVersion.status !== definition.status) {
            await tx.update(scenarioVersions).set({
              status: definition.status,
              contentHash: scenarioContentHash(definition),
              content: definition,
            }).where(eq(scenarioVersions.id, versionId));
          }
        } else {
          await tx.insert(scenarioVersions).values({
            id: versionId, scenarioId: definition.id, version: definition.version,
  ...
  async listRuns(ownerId: string) {
    const rows = await database().select().from(runs).where(and(eq(runs.ownerId, ownerId), eq(runs.stateFormat, "legacy_json"), isNull(runs.archivedAt))).orderBy(desc(runs.lastPlayedAt));
    return rows.map(toRun);
  }

  async listV10Runs(ownerId: string) {
    const db = database();
    const rows = await db.select().from(runs).where(and(eq(runs.ownerId, ownerId), eq(runs.stateFormat, "feature_heads_v10"), isNull(runs.archivedAt))).orderBy(desc(runs.lastPlayedAt));
    const result: V10RunRecord[] = [];
    for (const row of rows) {
      const [kernel] = await db.select().from(runKernelHeads).where(eq(runKernelHeads.runId, row.id)).limit(1);
      const featureRows = await db.select().from(runFeatureHeads).where(eq(runFeatureHeads.runId, row.id)).orderBy(asc(runFeatureHeads.featureId));
      if (!kernel) throw new Error("V10_KERNEL_HEAD_MISSING");
      result.push(composeV10Run(row, kernel, featureRows));
    }
    return result;
  }
  ...
  async createRun(ownerId: string, scenario: ScenarioDefinition, setupInput: unknown, seed: number, now: Date) {
    ...
    return database().transaction(async (tx) => {
      const [row] = await tx.insert(runs).values({ id, ownerId, scenarioVersionId: scenarioVersionId(scenario), title: setup.companyName, seed, engineVersion: ENGINE_VERSION, stateFormat: "legacy_json", stateVersion: 1, headEventSequence: 1, headState: state, headChecksum: checksum, status: state.status, createdAt: now, updatedAt: now, lastPlayedAt: now }).returning();
  ...
  async createV10Run(ownerId: string, scenario: ScenarioDefinition, setupInput: unknown, seed: number, now: Date) {
    ...
    const state = createInitialStateV10({ scenarioVersionId: scenarioVersionId(scenario), setup }, {
    ...
    return database().transaction(async (tx) => {
      const [row] = await tx.insert(runs).values({
        id, ownerId, scenarioVersionId: scenarioVersionId(scenario), title: setup.companyName,
        seed, engineVersion, stateFormat: "feature_heads_v10",
        stateVersion: state.kernel.version, headEventSequence: state.kernel.eventSequence,
        headState: null, headChecksum: state.kernel.overallChecksum, status: state.kernel.status,
        createdAt: now, updatedAt: now, lastPlayedAt: now,
      }).returning();
      await tx.insert(runKernelHeads).values({
        runId: id, kernel: state.kernel, manifest: state.manifest, version: state.kernel.version,
        overallChecksum: state.kernel.overallChecksum, updatedAt: now,
      });
      const featureRows = Object.entries(state.features).sort(([left], [right]) => left.localeCompare(right)).map(([featureId, head]) => ({
        runId: id, featureId, featureVersion: head.version, publicState: head.public,
        privateState: head.private, checksum: head.checksum, updatedAtVersion: head.updatedAtVersion, updatedAt: now,
      }));
      await tx.insert(runFeatureHeads).values(featureRows);
      for (const feature of featureRows) {
        await tx.insert(runFeatureBlobs).values({
          checksum: feature.checksum, featureId: feature.featureId, featureVersion: feature.featureVersion,
          publicState: feature.publicState, privateState: feature.privateState, createdAt: now,
        }).onConflictDoNothing();
      }
      const [snapshot] = await tx.insert(runV10Snapshots).values({
        runId: id, eventSequence: 0, stateVersion: 0, kernel: state.kernel, manifest: state.manifest,
        featureHeads: Object.fromEntries(featureRows.map((feature) => [feature.featureId, {
          checksum: feature.checksum, updatedAtVersion: feature.updatedAtVersion,
        }])),
        overallChecksum: state.kernel.overallChecksum, createdAt: now,
      }).returning();
      await tx.insert(runV10Checkpoints).values({runId:id,snapshotId:snapshot.id,eventSequence:0,name:"Run start",automatic:true,createdAt:now});
      return composeV10Run(row, {
        runId: id, kernel: state.kernel, manifest: state.manifest, version: state.kernel.version,
        overallChecksum: state.kernel.overallChecksum, updatedAt: now,
  ...
  async getRun(ownerId: string, runId: string) {
    const [row] = await database().select().from(runs).where(and(eq(runs.id, runId), eq(runs.ownerId, ownerId), eq(runs.stateFormat, "legacy_json"), isNull(runs.archivedAt))).limit(1);
    return row ? toRun(row) : null;
  }

  async getV10Run(ownerId: string, runId: string) {
    const db = database();
    const [row] = await db.select().from(runs).where(and(eq(runs.id, runId), eq(runs.ownerId, ownerId), eq(runs.stateFormat, "feature_heads_v10"), isNull(runs.archivedAt))).limit(1);
    if (!row) return null;
    const [kernel] = await db.select().from(runKernelHeads).where(eq(runKernelHeads.runId, runId)).limit(1);
    const featureRows = await db.select().from(runFeatureHeads).where(eq(runFeatureHeads.runId, runId)).orderBy(asc(runFeatureHeads.featureId));
    if (!kernel) throw new Error("V10_KERNEL_HEAD_MISSING");
    return composeV10Run(row, kernel, featureRows);
  }

  async executeCommand(ownerId: string, runId: string, request: CommandRequest, now: Date): Promise<CommandResponse> {
    return database().transaction(async (tx) => {
      const [row] = await tx.select().from(runs).where(and(eq(runs.id, runId), eq(runs.ownerId, ownerId), eq(runs.stateFormat, "legacy_json"), isNull(runs.archivedAt))).for("update").limit(1);
      if (!row) throw new Error("RUN_NOT_FOUND");
      if (!row.headState) throw new Error("LEGACY_RUN_STATE_MISSING");
      ...
      if (duplicate) {
        if (!duplicate.resultingState) throw new Error("LEGACY_DUPLICATE_STATE_MISSING");
  ...
  async executeV10Command(ownerId: string, runId: string, request: V10CommandRequest, now: Date): Promise<V10CommandResponse> {
    return database().transaction(async (tx) => {
      const [row] = await tx.select().from(runs).where(and(
        eq(runs.id, runId), eq(runs.ownerId, ownerId), eq(runs.stateFormat, "feature_heads_v10"), isNull(runs.archivedAt),
      )).for("update").limit(1);
      if (!row) throw new Error("RUN_NOT_FOUND");
      const [duplicate] = await tx.select().from(runCommands).where(and(
        eq(runCommands.runId, runId), eq(runCommands.clientCommandId, request.commandId),
      )).limit(1);
      if (duplicate) {
        if (!duplicate.responsePayload) throw new Error("V10_DUPLICATE_RESPONSE_MISSING");
        return structuredClone(duplicate.responsePayload) as V10CommandResponse;
      }
      if (row.stateVersion !== request.expectedVersion) throw new Error("VERSION_CONFLICT");

      const [kernelRow] = await tx.select().from(runKernelHeads).where(eq(runKernelHeads.runId, runId)).for("update").limit(1);
      if (!kernelRow) throw new Error("V10_KERNEL_HEAD_MISSING");
      const featureRows = await tx.select().from(runFeatureHeads).where(eq(runFeatureHeads.runId, runId))
        .orderBy(asc(runFeatureHeads.featureId)).for("update");
      const current = composeV10Run(row, kernelRow, featureRows);
      const command = engineCommandSchemaV10.parse({ ...request, actor: "player" });
      const result = applyCommandV10(current.state, command, { runId, now: now.toISOString() }, registryForEngineVersionV10(row.engineVersion));
      const storedEvents: PublicHistoryEventV10[] = result.response.events.map((event) => ({ ...event, id: `${runId}:${event.id}` }));
      const response: V10CommandResponse = {
        runId, version: result.state.kernel.version, checksum: result.state.kernel.overallChecksum,
        state: projectSimulationStateV10(result.state), events: storedEvents,
      ...
      await tx.insert(runCommands).values({
        runId, clientCommandId: request.commandId, expectedVersion: request.expectedVersion,
        resultingVersion: result.state.kernel.version, type: command.type, payload: command.payload,
        resultingState: null, responsePayload: response, resultingChecksum: result.state.kernel.overallChecksum,
        resultingEventSequence: result.state.kernel.eventSequence, createdAt: now,
      });
      if (storedEvents.length) await tx.insert(runV10Events).values(storedEvents.map((event) => ({
        id: event.id, runId, sequence: event.sequence, commandId: event.commandId,
        featureId: event.featureId, type: event.type, simulationDay: event.simulationDay,
        payload: event.payload, causality: event.causality, engineVersion: result.state.kernel.engineVersion, createdAt: now,
      })));
      await tx.update(runs).set({
        stateVersion: result.state.kernel.version, headEventSequence: result.state.kernel.eventSequence,
        headChecksum: result.state.kernel.overallChecksum, status: result.state.kernel.status,
        updatedAt: now, lastPlayedAt: now,
      }).where(eq(runs.id, runId));
      await tx.update(runKernelHeads).set({
        kernel: result.state.kernel, manifest: result.state.manifest, version: result.state.kernel.version,
        overallChecksum: result.state.kernel.overallChecksum, updatedAt: now,
      }).where(eq(runKernelHeads.runId, runId));
      for (const [featureId, head] of Object.entries(result.state.features).sort(([left], [right]) => left.localeCompare(right))) {
        await tx.update(runFeatureHeads).set({
          featureVersion: head.version, publicState: head.public, privateState: head.private,
          checksum: head.checksum, updatedAtVersion: head.updatedAtVersion, updatedAt: now,
        }).where(and(eq(runFeatureHeads.runId, runId), eq(runFeatureHeads.featureId, featureId)));
      }
      if (result.response.checkpointRequired) {
        for (const [featureId, head] of Object.entries(result.state.features)) {
          await tx.insert(runFeatureBlobs).values({
            checksum: head.checksum, featureId, featureVersion: head.version,
            publicState: head.public, privateState: head.private, createdAt: now,
          }).onConflictDoNothing();
        }
        const [snapshot] = await tx.insert(runV10Snapshots).values({
          runId, eventSequence: result.state.kernel.eventSequence, stateVersion: result.state.kernel.version,
          kernel: result.state.kernel, manifest: result.state.manifest,
          featureHeads: Object.fromEntries(Object.entries(result.state.features).map(([featureId, head]) => [featureId, {
            checksum: head.checksum, updatedAtVersion: head.updatedAtVersion,
          }])),
          overallChecksum: result.state.kernel.overallChecksum, createdAt: now,
        }).onConflictDoNothing().returning();
        if(snapshot)await tx.insert(runV10Checkpoints).values({runId,snapshotId:snapshot.id,eventSequence:snapshot.eventSequence,name:`Day ${result.state.kernel.simulationDay} irreversible decision`,automatic:true,createdAt:now});
      }
      return response;
    });
  }
  ...
  async resolvePendingV10CompetitorTurn(ownerId: string, runId: string, now: Date) {
    const run = await this.getV10Run(ownerId, runId);
    if (!run) throw new Error("RUN_NOT_FOUND");
    const envelope = pendingCompetitorDecisionEnvelopeV10(run.state);
    if (!envelope) return null;
    const db = database();
    await db.insert(runV10AgentTurns).values({
      runId, turnId: envelope.turnId, firmId: envelope.firmId, status: "pending",
      envelopeSchemaVersion: "competitor-decision-envelope-v1", envelope, plan: null,
      provider: null, model: null, promptVersion: envelope.promptVersion,
      latencyMs: null, inputTokens: null, outputTokens: null, fallbackReason: null,
      createdAt: now, completedAt: null,
    }).onConflictDoNothing();
    const [existing] = await db.select().from(runV10AgentTurns).where(and(eq(runV10AgentTurns.runId, runId), eq(runV10AgentTurns.turnId, envelope.turnId))).limit(1);
    if (!existing) throw new Error("V10_COMPETITOR_TURN_MISSING");
    if (existing.status === "completed") return toV10CompetitorTurn(existing);
    const generated = await generateCompetitorStrategicPlanV10(envelope);
    const completedAt = new Date();
    return db.transaction(async (tx) => {
      const [row] = await tx.select().from(runs).where(and(eq(runs.id, runId), eq(runs.ownerId, ownerId), eq(runs.stateFormat, "feature_heads_v10"), isNull(runs.archivedAt))).for("update").limit(1);
      if (!row) throw new Error("RUN_NOT_FOUND");
      const [turn] = await tx.select().from(runV10AgentTurns).where(and(eq(runV10AgentTurns.runId, runId), eq(runV10AgentTurns.turnId, envelope.turnId))).for("update").limit(1);
      if (!turn) throw new Error("V10_COMPETITOR_TURN_MISSING");
      if (turn.status === "completed") return toV10CompetitorTurn(turn);
      const [kernelRow] = await tx.select().from(runKernelHeads).where(eq(runKernelHeads.runId, runId)).for("update").limit(1);
      if (!kernelRow) throw new Error("V10_KERNEL_HEAD_MISSING");
      const featureRows = await tx.select().from(runFeatureHeads).where(eq(runFeatureHeads.runId, runId)).orderBy(asc(runFeatureHeads.featureId)).for("update");
      const current = composeV10Run(row, kernelRow, featureRows);
      ...
      if (!pending || pending.turnId !== envelope.turnId || pending.worldInputHash !== envelope.worldInputHash) {
        const [superseded] = await tx.update(runV10AgentTurns).set({ status: "superseded", fallbackReason: "STALE_COMPETITOR_TURN", completedAt }).where(eq(runV10AgentTurns.id, turn.id)).returning();
        return toV10CompetitorTurn(superseded);
      }
      ...
      const result = applyCommandV10(current.state, command, { runId, now: completedAt.toISOString() }, registryForEngineVersionV10(row.engineVersion));
      const storedEvents: PublicHistoryEventV10[] = result.response.events.map((event) => ({ ...event, id: `${runId}:${event.id}` }));
      ...
      await tx.insert(externalInputBlobs).values({
        contentHash, kind: "competitor_strategic_plan", schemaVersion: "competitor-strategic-plan-v1",
        payload: generated.plan, inputHash: envelope.worldInputHash, provider: generated.provider,
        model: generated.model, promptVersion: generated.promptVersion, createdAt: completedAt,
      }).onConflictDoNothing();
      await tx.insert(runV10ExternalInputRefs).values({
        runId, eventSequence: result.state.kernel.eventSequence, effectiveSimulationDay: result.state.kernel.simulationDay,
        contentHash, inheritedFromRunId: null, createdAt: completedAt,
      }).onConflictDoNothing();
      await tx.insert(runCommands).values({
        runId, clientCommandId: commandId, expectedVersion: current.stateVersion,
        resultingVersion: result.state.kernel.version, type: command.type, payload: command.payload,
        resultingState: null, responsePayload: null, resultingChecksum: result.state.kernel.overallChecksum,
        resultingEventSequence: result.state.kernel.eventSequence, createdAt: completedAt,
      });
      if (storedEvents.length) await tx.insert(runV10Events).values(storedEvents.map((event) => ({
        id: event.id, runId, sequence: event.sequence, commandId: event.commandId,
        featureId: event.featureId, type: event.type, simulationDay: event.simulationDay,
        payload: event.payload, causality: event.causality, engineVersion: result.state.kernel.engineVersion, createdAt: completedAt,
      })));
      await tx.update(runs).set({
        stateVersion: result.state.kernel.version, headEventSequence: result.state.kernel.eventSequence,
        headChecksum: result.state.kernel.overallChecksum, status: result.state.kernel.status,
        updatedAt: completedAt, lastPlayedAt: completedAt,
      }).where(eq(runs.id, runId));
      await tx.update(runKernelHeads).set({
        kernel: result.state.kernel, manifest: result.state.manifest, version: result.state.kernel.version,
        overallChecksum: result.state.kernel.overallChecksum, updatedAt: completedAt,
      }).where(eq(runKernelHeads.runId, runId));
      for (const [featureId, head] of Object.entries(result.state.features).sort(([left], [right]) => left.localeCompare(right))) {
        await tx.update(runFeatureHeads).set({
          featureVersion: head.version, publicState: head.public, privateState: head.private,
          checksum: head.checksum, updatedAtVersion: head.updatedAtVersion, updatedAt: completedAt,
        }).where(and(eq(runFeatureHeads.runId, runId), eq(runFeatureHeads.featureId, featureId)));
        await tx.insert(runFeatureBlobs).values({
          checksum: head.checksum, featureId, featureVersion: head.version,
          publicState: head.public, privateState: head.private, createdAt: completedAt,
        }).onConflictDoNothing();
      }
      const [snapshot] = await tx.insert(runV10Snapshots).values({
        runId, eventSequence: result.state.kernel.eventSequence, stateVersion: result.state.kernel.version,
        kernel: result.state.kernel, manifest: result.state.manifest,
        featureHeads: Object.fromEntries(Object.entries(result.state.features).map(([featureId, head]) => [featureId, { checksum: head.checksum, updatedAtVersion: head.updatedAtVersion }])),
        overallChecksum: result.state.kernel.overallChecksum, createdAt: completedAt,
      }).onConflictDoNothing().returning();
      if (snapshot) await tx.insert(runV10Checkpoints).values({
        runId, snapshotId: snapshot.id, eventSequence: snapshot.eventSequence,
        name: `Before ${envelope.firmId} board plan`, automatic: true, createdAt: completedAt,
      });
      const [finished] = await tx.update(runV10AgentTurns).set({
        status: "completed", plan: generated.plan, provider: generated.provider, model: generated.model,
        promptVersion: generated.promptVersion, latencyMs: generated.latencyMs,
        inputTokens: generated.inputTokens, outputTokens: generated.outputTokens,
        fallbackReason: generated.fallbackReason, completedAt,
      }).where(eq(runV10AgentTurns.id, turn.id)).returning();
      return toV10CompetitorTurn(finished);
    });
  }
  ...
  async getV10CompetitorTurn(ownerId: string, runId: string, turnId: string) {
    const run = await this.getV10Run(ownerId, runId);
    if (!run) throw new Error("RUN_NOT_FOUND");
    const [row] = await database().select().from(runV10AgentTurns).where(and(eq(runV10AgentTurns.runId, runId), eq(runV10AgentTurns.turnId, turnId))).limit(1);
    return row ? toV10CompetitorTurn(row) : null;
  }

  async listV10ExternalInputs(ownerId: string, runId: string): Promise<V10ExternalInputRecord[]> {
    const run = await this.getV10Run(ownerId, runId);
    if (!run) throw new Error("RUN_NOT_FOUND");
    const rows = await database().select({ ref: runV10ExternalInputRefs, blob: externalInputBlobs })
      .from(runV10ExternalInputRefs).innerJoin(externalInputBlobs, eq(runV10ExternalInputRefs.contentHash, externalInputBlobs.contentHash))
      .where(eq(runV10ExternalInputRefs.runId, runId)).orderBy(asc(runV10ExternalInputRefs.eventSequence));
    return rows.map(({ ref, blob }) => ({
      contentHash: blob.contentHash, runId: ref.runId, eventSequence: ref.eventSequence,
      effectiveSimulationDay: ref.effectiveSimulationDay, kind: blob.kind as V10ExternalInputRecord["kind"],
      payload: blob.payload, inputHash: blob.inputHash, provider: blob.provider as V10ExternalInputRecord["provider"],
      model: blob.model, promptVersion: blob.promptVersion, inheritedFromRunId: ref.inheritedFromRunId,
      observedAt: ref.createdAt.toISOString(),
    }));
  }

  async listV10Events(ownerId:string,runId:string,options:{featureId?:string;cursor?:number;limit?:number}={}){
    const owned=await this.getV10Run(ownerId,runId);if(!owned)throw new Error("RUN_NOT_FOUND");const limit=Math.min(100,Math.max(1,options.limit??50));
    const base=options.featureId?and(eq(runV10Events.runId,runId),eq(runV10Events.featureId,options.featureId)):eq(runV10Events.runId,runId);
    const where=options.cursor?and(base,lt(runV10Events.sequence,options.cursor)):base;
    const rows=await database().select().from(runV10Events).where(where).orderBy(desc(runV10Events.sequence)).limit(limit);
    const events:PublicHistoryEventV10[]=rows.map((row)=>({id:row.id,sequence:row.sequence,commandId:row.commandId??"system",type:row.type as `${string}.${string}`,featureId:row.featureId,simulationDay:row.simulationDay,payload:row.payload,causality:row.causality??undefined}));
    return {events,nextCursor:events.length===limit?events.at(-1)?.sequence??null:null};
  }

  async listV10Checkpoints(ownerId:string,runId:string){
    const owned=await this.getV10Run(ownerId,runId);if(!owned)throw new Error("RUN_NOT_FOUND");
    const rows=await database().select({checkpoint:runV10Checkpoints,snapshot:runV10Snapshots}).from(runV10Checkpoints).innerJoin(runV10Snapshots,eq(runV10Checkpoints.snapshotId,runV10Snapshots.id)).where(eq(runV10Checkpoints.runId,runId)).orderBy(desc(runV10Checkpoints.eventSequence));
    return rows.map(({checkpoint,snapshot}):CheckpointRecord=>({id:checkpoint.id,runId,eventSequence:checkpoint.eventSequence,stateVersion:snapshot.stateVersion,name:checkpoint.name,automatic:checkpoint.automatic,checksum:snapshot.overallChecksum,createdAt:checkpoint.createdAt.toISOString()}));
  }

  async createV10Checkpoint(ownerId:string,runId:string,name:string,now:Date){
    return database().transaction(async(tx)=>{
      const [run]=await tx.select().from(runs).where(and(eq(runs.id,runId),eq(runs.ownerId,ownerId),eq(runs.stateFormat,"feature_heads_v10"))).for("update").limit(1);if(!run)throw new Error("RUN_NOT_FOUND");
      let [snapshot]=await tx.select().from(runV10Snapshots).where(and(eq(runV10Snapshots.runId,runId),eq(runV10Snapshots.eventSequence,run.headEventSequence))).limit(1);
      if(!snapshot){const [kernel]=await tx.select().from(runKernelHeads).where(eq(runKernelHeads.runId,runId)).for("update").limit(1);if(!kernel)throw new Error("V10_KERNEL_HEAD_MISSING");const features=await tx.select().from(runFeatureHeads).where(eq(runFeatureHeads.runId,runId)).orderBy(asc(runFeatureHeads.featureId)).for("update");
        for(const feature of features)await tx.insert(runFeatureBlobs).values({checksum:feature.checksum,featureId:feature.featureId,featureVersion:feature.featureVersion,publicState:feature.publicState,privateState:feature.privateState,createdAt:now}).onConflictDoNothing();
        [snapshot]=await tx.insert(runV10Snapshots).values({runId,eventSequence:run.headEventSequence,stateVersion:run.stateVersion,kernel:kernel.kernel,manifest:kernel.manifest,featureHeads:Object.fromEntries(features.map((feature)=>[feature.featureId,{checksum:feature.checksum,updatedAtVersion:feature.updatedAtVersion}])),overallChecksum:kernel.overallChecksum,createdAt:now}).returning();}
      const [checkpoint]=await tx.insert(runV10Checkpoints).values({runId,snapshotId:snapshot.id,eventSequence:snapshot.eventSequence,name,automatic:false,createdAt:now}).returning();
      return {id:checkpoint.id,runId,eventSequence:checkpoint.eventSequence,stateVersion:snapshot.stateVersion,name,automatic:false,checksum:snapshot.overallChecksum,createdAt:checkpoint.createdAt.toISOString()};
    });
  }

  async forkV10Run(ownerId:string,runId:string,checkpointId:string,now:Date){
    return database().transaction(async(tx)=>{
      const [source]=await tx.select().from(runs).where(and(eq(runs.id,runId),eq(runs.ownerId,ownerId),eq(runs.stateFormat,"feature_heads_v10"))).limit(1);if(!source)throw new Error("RUN_NOT_FOUND");
      const [found]=await tx.select({checkpoint:runV10Checkpoints,snapshot:runV10Snapshots}).from(runV10Checkpoints).innerJoin(runV10Snapshots,eq(runV10Checkpoints.snapshotId,runV10Snapshots.id)).where(and(eq(runV10Checkpoints.id,checkpointId),eq(runV10Checkpoints.runId,runId))).limit(1);if(!found)throw new Error("CHECKPOINT_NOT_FOUND");
      const checksums=Object.values(found.snapshot.featureHeads).map((head)=>head.checksum);const blobs=await tx.select().from(runFeatureBlobs).where(inArray(runFeatureBlobs.checksum,checksums));
      const features:SimulationStateV10["features"]={};for(const [featureId,metadata] of Object.entries(found.snapshot.featureHeads)){const blob=blobs.find((item)=>item.checksum===metadata.checksum&&item.featureId===featureId);if(!blob)throw new Error(`V10_FEATURE_BLOB_MISSING:${featureId}`);features[featureId]={version:blob.featureVersion,public:blob.publicState,private:blob.privateState,checksum:blob.checksum,updatedAtVersion:metadata.updatedAtVersion};}
      const state:SimulationStateV10={kernel:found.snapshot.kernel,manifest:found.snapshot.manifest,features};const id=randomUUID();
      const [row]=await tx.insert(runs).values({id,ownerId,scenarioVersionId:source.scenarioVersionId,parentRunId:source.id,title:`${source.title} — fork`,status:state.kernel.status,seed:source.seed,engineVersion:source.engineVersion,stateFormat:"feature_heads_v10",stateVersion:state.kernel.version,headEventSequence:state.kernel.eventSequence,headState:null,headChecksum:state.kernel.overallChecksum,createdAt:now,updatedAt:now,lastPlayedAt:now}).returning();
      const [kernel]=await tx.insert(runKernelHeads).values({runId:id,kernel:state.kernel,manifest:state.manifest,version:state.kernel.version,overallChecksum:state.kernel.overallChecksum,updatedAt:now}).returning();
      const featureRows=Object.entries(features).sort(([left],[right])=>left.localeCompare(right)).map(([featureId,head])=>({runId:id,featureId,featureVersion:head.version,publicState:head.public,privateState:head.private,checksum:head.checksum,updatedAtVersion:head.updatedAtVersion,updatedAt:now}));await tx.insert(runFeatureHeads).values(featureRows);
      const inheritedInputs=await tx.select().from(runV10ExternalInputRefs).where(and(eq(runV10ExternalInputRefs.runId,runId),lte(runV10ExternalInputRefs.eventSequence,found.checkpoint.eventSequence)));
      if(inheritedInputs.length)await tx.insert(runV10ExternalInputRefs).values(inheritedInputs.map((input)=>({runId:id,eventSequence:input.eventSequence,effectiveSimulationDay:input.effectiveSimulationDay,contentHash:input.contentHash,inheritedFromRunId:runId,createdAt:now})));
      const inheritedEvents=await tx.select().from(runV10Events).where(and(eq(runV10Events.runId,runId),lte(runV10Events.sequence,found.checkpoint.eventSequence))).orderBy(asc(runV10Events.sequence));
      if(inheritedEvents.length)await tx.insert(runV10Events).values(inheritedEvents.map((event)=>({
        id:`${id}:${event.id.startsWith(`${runId}:`)?event.id.slice(runId.length+1):event.id}`,runId:id,sequence:event.sequence,commandId:event.commandId,
        featureId:event.featureId,type:event.type,simulationDay:event.simulationDay,payload:event.payload,causality:event.causality,
        engineVersion:event.engineVersion,createdAt:now,
      })));
      const [snapshot]=await tx.insert(runV10Snapshots).values({runId:id,eventSequence:state.kernel.eventSequence,stateVersion:state.kernel.version,kernel:state.kernel,manifest:state.manifest,featureHeads:found.snapshot.featureHeads,overallChecksum:state.kernel.overallChecksum,createdAt:now}).returning();await tx.insert(runV10Checkpoints).values({runId:id,snapshotId:snapshot.id,eventSequence:snapshot.eventSequence,name:"Fork start",automatic:true,createdAt:now});
      return composeV10Run(row,kernel,featureRows);
    });
  }
  ...
  async resolvePendingAgentTurn(ownerId: string, runId: string, now: Date) {
    ...
    return db.transaction(async (tx) => {
      const [locked] = await tx.select().from(runs).where(and(eq(runs.id, runId), eq(runs.ownerId, ownerId), eq(runs.stateFormat, "legacy_json"), isNull(runs.archivedAt))).for("update").limit(1);
      ...
      if (!locked.headState) throw new Error("LEGACY_RUN_STATE_MISSING");
  ...
  async publishMarketDossier(dossier: MarketDossierVersion, metadata: { provider: "openai" | "authored"; model?: string; promptVersion: string }, now: Date) {
    ...
    const candidates = await db.select({ id: runs.id }).from(runs).where(and(inArray(runs.scenarioVersionId, versions.map((item) => item.id)), eq(runs.status, "active"), eq(runs.stateFormat, "legacy_json"), isNull(runs.archivedAt)));
    let updatedRuns = 0;
    for (const candidate of candidates) await db.transaction(async (tx) => {
      ...
      if (!run || !run.headState || run.headState.schemaVersion !== 3 || run.headState.externalInputRefs?.some((item) => item.kind === "market_dossier" && item.inputHash === dossier.contentHash)) return;
    ...
    const dossierBlobHash = stateChecksum({ kind: "market_dossier", dossier });
    const v10Candidates = await db.select({ id: runs.id }).from(runs).where(and(
      inArray(runs.scenarioVersionId, versions.map((item) => item.id)),
      eq(runs.status, "active"),
      eq(runs.stateFormat, "feature_heads_v10"),
      inArray(runs.engineVersion, [V10_1_ENGINE_VERSION, V10_2_ENGINE_VERSION, V10_3_ENGINE_VERSION]),
      isNull(runs.archivedAt),
    ));
    for (const candidate of v10Candidates) await db.transaction(async (tx) => {
      ...
      if (!run) return;
      const [duplicate] = await tx.select().from(runV10ExternalInputRefs).where(and(
        eq(runV10ExternalInputRefs.runId, run.id),
        eq(runV10ExternalInputRefs.contentHash, dossierBlobHash),
      )).limit(1);
      if (duplicate) return;
      const [kernelRow] = await tx.select().from(runKernelHeads)
        .where(eq(runKernelHeads.runId, run.id)).for("update").limit(1);
      if (!kernelRow) throw new Error("V10_KERNEL_HEAD_MISSING");
      const featureRows = await tx.select().from(runFeatureHeads)
        .where(eq(runFeatureHeads.runId, run.id))
        .orderBy(asc(runFeatureHeads.featureId)).for("update");
      const current = composeV10Run(run, kernelRow, featureRows);
      if (pendingCompetitorDecisionEnvelopeV10(current.state)) return;
      const externalInputId = randomUUID();
      const command = engineCommandSchemaV10.parse({
      ...
      const storedEvents = result.response.events.map((event) => ({
        ...event,
        id: `${run.id}:${event.id}`,
      }));
      await tx.insert(externalInputBlobs).values({
        contentHash: dossierBlobHash,
        kind: "market_dossier",
        schemaVersion: "market-dossier-v9.1",
        payload: dossier,
        inputHash: dossier.contentHash,
      ...
      await tx.insert(runCommands).values({
        ...
        expectedVersion: current.stateVersion,
        resultingVersion: result.state.kernel.version,
        type: command.type,
        payload: command.payload,
        resultingState: null,
        responsePayload: null,
        resultingChecksum: result.state.kernel.overallChecksum,
        resultingEventSequence: result.state.kernel.eventSequence,
        createdAt: now,
      });
      if (storedEvents.length) await tx.insert(runV10Events).values(storedEvents.map((event) => ({
        id: event.id,
        ...
        engineVersion: result.state.kernel.engineVersion,
        createdAt: now,
      })));
      await tx.update(runs).set({
        stateVersion: result.state.kernel.version,
        headEventSequence: result.state.kernel.eventSequence,
        headChecksum: result.state.kernel.overallChecksum,
        status: result.state.kernel.status,
        updatedAt: now,
      }).where(eq(runs.id, run.id));
      await tx.update(runKernelHeads).set({
        kernel: result.state.kernel,
        manifest: result.state.manifest,
        version: result.state.kernel.version,
        overallChecksum: result.state.kernel.overallChecksum,
        updatedAt: now,
      }).where(eq(runKernelHeads.runId, run.id));
      for (const [featureId, head] of Object.entries(result.state.features).sort(([left], [right]) => left.localeCompare(right))) {
        await tx.update(runFeatureHeads).set({
          featureVersion: head.version,
          publicState: head.public,
          privateState: head.private,
          checksum: head.checksum,
          updatedAtVersion: head.updatedAtVersion,
          updatedAt: now,
        }).where(and(eq(runFeatureHeads.runId, run.id), eq(runFeatureHeads.featureId, featureId)));
        await tx.insert(runFeatureBlobs).values({
          checksum: head.checksum,
          featureId,
          featureVersion: head.version,
      ...
      if (snapshot) await tx.insert(runV10Checkpoints).values({
        ...
        name: `Market dossier ${dossier.capturedAt.slice(0, 10)}`,
        automatic: true,
        createdAt: now,
      });
      updatedRuns += 1;
  ...
  async createCheckpoint(ownerId: string, runId: string, name: string, now: Date) {
    return database().transaction(async (tx) => {
      const [run] = await tx.select().from(runs).where(and(eq(runs.id, runId), eq(runs.ownerId, ownerId), eq(runs.stateFormat, "legacy_json"))).for("update").limit(1);
      if (!run || !run.headState) throw new Error("RUN_NOT_FOUND");
  ...
  async saveDialogue(ownerId: string, runId: string, turn: DialogueTurn) {
    const owned = await this.getRun(ownerId, runId)??await this.getV10Run(ownerId,runId); if (!owned) throw new Error("RUN_NOT_FOUND");
